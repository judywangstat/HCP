...
results_matrix <- matrix(NA, nrow = length(B_values), ncol = length(n_values) * 3)
rownames(results_matrix) <- paste0("B=", B_values)
...
for (i in seq_along(B_values)) {
  ...
  for (j in seq_along(n_values)) {
    ...
    assign("B_sampling", B, envir = .GlobalEnv)
    ...
    if (length(hcp_vals) == 3) {
      ...
      warning(paste("Unexpected output format for B =", B, "and n =", n))
    }
  }
}
...
cover_plot <- function(data, sample_size, y_var) {
  ggplot(data, aes(x = B)) + 
    ...
    labs(x = "B", y = "Marginal Coverage", title = paste("n =", sample_size)) +
    ylim(0.8, 1.0) +
...
leng_plot <- function(data, sample_size, y_col, sd_col) {
  ggplot(data) +
    geom_line(aes_string(x = "B", y = y_col), color = "blue") +
    geom_ribbon(aes_string(x = "B", ymin = paste0(y_col, " - 1.645 * ", sd_col), ymax = paste0(y_col, " + 1.645 * ", sd_col)), 
                fill = "blue", alpha = 0.2) +
...
ggsave(filename = file.path(main_dir, "sens_B_reproduce.pdf"), plot = plot_grid, width = 7, height = 5)
