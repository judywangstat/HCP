...
output_matrix_5 <- run_HCP_local(nclusters = 5)
combined_matrix <- rbind(output_matrix_1, output_matrix_5)
rownames(combined_matrix) <- c("Coverage_K1", "Length_K1", "Coverage_K5", "Length_K5")
...
grid_data$coverage_prob_K5  <- output_matrix_5[1, ] # Third row: Coverage for K=5
grid_data$interval_length_K5 <- output_matrix_5[2, ]   # Fourth row: Length for K=5


limits_range_cover <- range(c(grid_data$coverage_prob_K1, grid_data$coverage_prob_K5), na.rm = TRUE)
limits_range_leng <- range(c(grid_data$interval_length_K1, grid_data$interval_length_K5), na.rm = TRUE)
...
p3 <- ggplot(grid_data, aes(x = x2, y = x3, fill = coverage_prob_K5)) +
  geom_tile(color = "white") +
  scale_fill_viridis(option = "D", direction = 1, limits = limits_range_cover) +
  theme_minimal(base_size = 8) +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(x = expression(x[2]), y = expression(x[3]), title = "Local Coverage, K=5", fill = " ")

p4 <- ggplot(grid_data, aes(x = x2, y = x3, fill = interval_length_K5)) +
  geom_tile(color = "white") +
  scale_fill_viridis(option = "D", direction = 1, limits = limits_range_leng) +
  theme_minimal(base_size = 8) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  labs(x = expression(x[2]), y = expression(x[3]), title = "Local Length, K=5", fill = "")


plot_grid = grid.arrange(p1, p3, p2, p4, nrow = 2, ncol = 2)

ggsave(filename = file.path(main_dir, "local_100_reproduce.pdf"), plot = plot_grid, width = 7, height = 5)
