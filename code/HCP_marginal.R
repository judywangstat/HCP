...
b_true = 6  # Number of quantile levels
B_sampling = 5
S_splitting = 1
alpha = 0.1
n_grid = 200
...
for (n in c(100, 300, 500)) {
  ...
  for (scenario in c("Bimo-fix")) {
    ...
    results <- foreach(
      ...
      if (!(length(train_multi_index) == length(cali_multi_index) && length(cali_multi_index) == S_splitting)) {
        stop("length(train_multi_index), length(cali_multi_index), and S_splitting are not equal. Program has been stopped.")
      }
      
      p_values_S_group <- vector("list", S_splitting)
      unwt_p_values_S_group <- vector("list", S_splitting)
      ...
      for (ii in 1:S_splitting) {
        ...
        unwt_p_values_S_group[[ii]] <- matrix(0, nrow = n_simultaneous, ncol = n_grid)  
      }
      ...
      for (s_split in 1:S_splitting) {
        ...
        for (b_sampling in 1:B_sampling) {
          ...
          for (same in 1: n_simultaneous) {
            p_values[[same]][b_sampling, ] = p_value(-dens_Pyx_LQR_ca,
                                                     ...
                                                     wt_test[same], n_grid)
            unwt_p_values[[same]][b_sampling, ] = p_value(-dens_Pyx_LQR_ca,
                                                          rep(1, length(wt_cali)) ,-dens_Pyx_LQR_te[same,],
                                                          1, n_grid)
          }
        }
        ...
        for (same in 1:n_simultaneous) {
          ...
          unwt_p_values_S_group[[s_split]][same,] = CCT_p_value_matrix(unwt_p_values[[same]], omega_B_sampling)
        }
      }
      ...
      for (same in 1: n_simultaneous) {
        ...
        p_S_group = do.call(rbind, lapply(p_values_S_group, function(x) x[same, ]))  
    ...
    print(colMeans(pointwise_result))
    print(apply(pointwise_result, 2, sd)/sqrt(num_simulations))
