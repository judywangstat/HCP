...
for(n_val in sample_sizes) {
  for(scen in scenarios) {
    ...
    out <- capture.output(source("~/Desktop/HCP_code/Table S.3/HCP_compare_score.R", local = TRUE))
...
write.csv(final_results, file = "~/Desktop/HCP_code/Table S.3/final_results.csv", row.names = FALSE)