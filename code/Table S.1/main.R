...
for(n_val in sample_sizes) {
  for(scen in scenarios) {
    ...
    hcp_out <- capture.output(source("~/Desktop/HCP_code/Table S.1/HCP_marginal.R", local = TRUE))
    hcp_vals <-  as.numeric(strsplit(trimws(hcp_out), "\\s+")[[1]])
    
    dwr_out <- capture.output(source("~/Desktop/HCP_code/Table S.1/DWR.R", local = TRUE))
    dwr_vals <- as.numeric(strsplit(trimws(dwr_out), "\\s+")[[1]])
    
    lc_out <- capture.output(source("~/Desktop/HCP_code/Table S.1/LC.R", local = TRUE))
    lc_vals <- as.numeric(strsplit(trimws(lc_out), "\\s+")[[1]])
    
    lmem_out <- capture.output(source("~/Desktop/HCP_code/Table S.1/LMEM.R", local = TRUE))
...
write.csv(final_results, file = "~/Desktop/HCP_code/Table S.1/final_results.csv", row.names = FALSE)
