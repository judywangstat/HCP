...
for (pred in prediction_types) {
  for (metric in metrics) {
    for (i in seq_along(methods)) {
      ...
      full_script_path <- file.path("~/Desktop/HCP_code/Table S.2", script_file)
      out <- capture.output(source(full_script_path, local = TRUE))
...
print(final_table)
write.csv(final_table, file = "~/Desktop/HCP_code/Table S.2/final_results.csv", row.names = TRUE)