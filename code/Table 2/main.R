...
for(n_val in sample_sizes) {
  ...
  for(scen in scenarios) {
    ...
    out <- capture.output(source("~/Desktop/HCP_code/Table 2/HCP_simul20.R", local = TRUE))
...
for(n_val in sample_sizes) {
  ...
  for(scen in scenarios) {
    ...
    out <- capture.output(source("~/Desktop/HCP_code/Table 2/HCP_simul50.R", local = TRUE))
    vals <- as.numeric(strsplit(trimws(out), "\\s+")[[1]])
...
print(final_results)
write.csv(final_results, file = "~/Desktop/HCP_code/Table 2/final_results.csv", row.names = FALSE)
