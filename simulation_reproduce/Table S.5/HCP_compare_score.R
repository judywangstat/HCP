...
results <- foreach(
  ...
  for (s_split in 1:S_splitting) {
    ...
    ### Calculate weights on test points
    if (wt_model == "grf") {
    ...
    for (b_sampling in 1:B_sampling) {
      ...
      for (same in 1:n_simultaneous) {
        p_values[[same]][b_sampling, ] = p_value(-dens_Pyx_LQR_ca,
                                                 ...
                                                 wt_test[same],
                                                 n_grid)
        res_p_values[[same]][b_sampling, ] = p_value(score_cali, wt_cali, score_test[same, ], wt_test[same], n_grid)
      }
    }
  ...
  for (same in 1:n_simultaneous) {
    ...
    res_p_K_group = do.call(rbind, lapply(res_p_values_K_group, function(x)
      x[same, ]))
    ...
    res_in_interval <- (Y_test[same] >= res_lo) &
      (Y_test[same] <= res_hi)
    res_result_length = res_hi - res_lo
    
    each_result[same, ] = c(in_interval,
                            result_length,
                            res_in_interval,
                            res_result_length)
  }
